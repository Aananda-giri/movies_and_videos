...
def home(request):
    ...
    trending_all = requests.get(BASE_URL + '/trending/all/week?' + API_KEY).json()['results']
    # print(trending_all)
    popular_movie = requests.get(BASE_URL + '/movie/popular?' + API_KEY + '&language=en-US&page=1').json()['results']
    popular_series = requests.get(BASE_URL + '/tv/popular?' + API_KEY + '&language=en-US&page=1').json()['results']
    latest_movie = requests.get(BASE_URL + '/movie/now_playing?' + API_KEY + '&language=en-US&page=1').json()['results']
    latest_series = requests.get(BASE_URL + '/tv/on_the_air?' + API_KEY + '&language=en-US&page=1').json()['results']
    upcomming = requests.get(BASE_URL + '/movie/upcoming?' + API_KEY + '&language=en-US&page=1').json()['results']
...
def search(request):
    ...
    if search_term !=None:
        ...
        search_results = requests.get(f'https://api.themoviedb.org/3/search/multi?api_key=<<TMDB_API_KEY>>&language=en-US&page=1&include_adult=true&query={search_term}').json()['results']
        ...
        # t/p/w500/pHkKbIRoCe7zIFvqan9LFSaQAde.jpg
        search_results = requests.get(f'https://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&{API_KEY}&with_genres={genre_parsed}').json()['results']
...
def series(request):
    ...
    season_data = requests.get(f'https://api.themoviedb.org/3/tv/{tmdb_id}/season/{season}?api_key=<<TMDB_API_KEY>>&language=en-US')
    print(season_data.json())
    
    # for similar series
    # https://api.themoviedb.org/3/tv/60573/similar?api_key=<<TMDB_API_KEY>>&language=en-US&page=1
    similar = requests.get(f'https://api.themoviedb.org/3/tv/{tmdb_id}/similar?api_key=<<TMDB_API_KEY>>&language=en-US&page=1').json()
    ...
    # print(f'\n movie_url: {movie_url}')
    series_data = requests.get(f'https://api.themoviedb.org/3/tv/{tmdb_id}?api_key=<<TMDB_API_KEY>>&language=en-US').json()
...
def movie(request):
    ...
    if request.method == 'GET':
        ...
        movie_data = requests.get(f'https://api.themoviedb.org/3/movie/{tmdb_id}?api_key=<<TMDB_API_KEY>>&language=en-US').json()
        ...
        similar = requests.get(f'https://api.themoviedb.org/3/movie/{tmdb_id}/similar?api_key=<<TMDB_API_KEY>>&language=en-US&page=1').json()
...
def trailer(request):
    if request.method == 'GET':
        ...
        if what == 'movie':
            res = requests.get(f'https://api.themoviedb.org/3/movie/{tmdb_id}/videos?api_key=<<TMDB_API_KEY>>').json()
        else:
            res = requests.get(f'https://api.themoviedb.org/3/tv/{tmdb_id}/videos?api_key=<<TMDB_API_KEY>>').json()
